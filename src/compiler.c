...
// Symbol Tables
struct { char name[MAX_IDENTIFIER]; int addr; int type_id; bool is_array; } globals[MAX_GLOBALS];
int global_count = 0;
struct { char name[MAX_IDENTIFIER]; int offset; int type_id; bool is_array; } locals[MAX_GLOBALS];
int local_count = 0;

struct { char name[MAX_IDENTIFIER]; int addr; } funcs[MAX_GLOBALS];
int func_count = 0;
...
int find_local(char* name) { for(int i=0; i<local_count; i++) if(strcmp(locals[i].name, name)==0) return i; return -1; }
int find_global(char* name) { for(int i=0; i<global_count; i++) if(strcmp(globals[i].name, name)==0) return i; return -1; }
...
void push_loop() {
    if (loop_depth >= MAX_LOOP_NESTING) { printf("Error: Loop nesting too deep\n"); exit(1); }
...
void emit_break() {
    if (loop_depth == 0) { printf("Error: 'break' outside of loop\n"); exit(1); }
    LoopControl* loop = &loop_stack[loop_depth - 1];
    if (loop->break_count >= MAX_JUMPS_PER_LOOP) { printf("Error: Too many 'break' statements\n"); exit(1); }
    emit(OP_JMP);
...
void emit_continue() {
    if (loop_depth == 0) { printf("Error: 'continue' outside of loop\n"); exit(1); }
    LoopControl* loop = &loop_stack[loop_depth - 1];
    if (loop->continue_count >= MAX_JUMPS_PER_LOOP) { printf("Error: Too many 'continue' statements\n"); exit(1); }
    emit(OP_JMP);
...
void next_token() {
    while (1) {
        ...
        while (isspace(c)) { src++; c = (unsigned char)*src; }
        if (*src == '/' && *(src + 1) == '/') { while (*src != '\n' && *src != '\0') src++; } else break;
    }

    if (*src == 'f' && *(src + 1) == '"') {
        src += 2; char* start = src; int brace_depth = 0;
        while (*src) {
        ...
        int len = (int)(src - start); if (len > MAX_IDENTIFIER-1) len = MAX_IDENTIFIER-1;
        strncpy(curr.text, start, len); curr.text[len] = '\0';
        if (*src == '"') src++; curr.type = TK_FSTR; return;
    }

    if (*src == 0) { curr.type = TK_EOF; return; }

    if (isdigit((unsigned char)*src) || (*src == '.' && isdigit((unsigned char)*(src+1)))) {
        curr.type = TK_NUM; char* end; curr.val_float = strtod(src, &end);
        if (end > src && *(end-1) == '.' && *end == '.') end--;
        int len = (int)(end - src); if (len > MAX_IDENTIFIER-1) len = MAX_IDENTIFIER-1;
        strncpy(curr.text, src, len); curr.text[len] = '\0';
        src = end; return;
    }

    if (isalpha((unsigned char)*src)) {
        char* start = src; while (isalnum((unsigned char)*src) || *src == '_') src++;
        int len = (int)(src - start); if (len > MAX_IDENTIFIER-1) len = MAX_IDENTIFIER-1;
        strncpy(curr.text, start, len); curr.text[len] = '\0';
    ...
    if (*src == '"') {
        src++; char* start = src; while (*src != '"' && *src != 0) src++;
        int len = (int)(src - start); if (len > MAX_IDENTIFIER-1) len = MAX_IDENTIFIER-1;
        strncpy(curr.text, start, len); curr.text[len] = '\0';
        if (*src == '"') src++; curr.type = TK_STR; return;
    }
    ...
    switch (*src++) {
        ...
        case '!': if (*src == '=') { src++; curr.type = TK_NEQ; } else { printf("Error: Unexpected char '!'\n"); exit(1); } break;
...
void match(MyloTokenType t) {
    ...
    else { printf("Error: Expected token type %d, got %d (Text: %s)\n", t, curr.type, curr.text); exit(1); }
}
...
void factor() {
    if (curr.type == TK_NUM) { int idx = make_const(curr.val_float); emit(OP_PSH_NUM); emit(idx); match(TK_NUM); }
        ...
        match(TK_MINUS);
        if (curr.type == TK_NUM) { int idx = make_const(-curr.val_float); emit(OP_PSH_NUM); emit(idx); match(TK_NUM); }
        else { emit(OP_PSH_NUM); emit(make_const(0.0)); factor(); emit(OP_SUB); }
    }

    else if (curr.type == TK_LBRACKET) { // Array Literal
        match(TK_LBRACKET);
        int count = 0;
        if (curr.type != TK_RBRACKET) { expression(); count++; while (curr.type == TK_COMMA) { match(TK_COMMA); expression(); count++; } }
        ...
        while (*end && braces > 0) { if (*end == '{') braces++; if (*end == '}') braces--; if (braces > 0) end++; }
        ...
        while (*ptr) {
            if (*ptr == '{') {
                if (ptr > start) {
                    ...
                    strncpy(chunk, start, len); chunk[len] = '\0'; int id = make_string(chunk); emit(OP_PSH_STR); emit(id); emit(OP_CAT);
                }
                ptr++; char* expr_start = ptr; while (*ptr && *ptr != '}') ptr++;
                if (*ptr == '}') { char expr_code[256]; int len = (int)(ptr - expr_start); strncpy(expr_code, expr_start, len); expr_code[len] = '\0'; char* old_src = src; Token old_token = curr; src = expr_code; next_token(); expression(); src = old_src; curr = old_token; emit(OP_CAT); ptr++; start = ptr; }
            } else ptr++;
        }
        if (ptr > start) {
            char chunk[MAX_STRING_LENGTH]; int len = (int)(ptr - start);
        ...
        if (curr.type == TK_LPAREN) {
            ...
            if (loc != -1) { emit(OP_LVAR); emit(locals[loc].offset); type_id = locals[loc].type_id; is_array = locals[loc].is_array; }
                 ...
                 if (glob == -1) { printf("Error: Undefined var '%s'\n", name); exit(1); }
            ...
            while (curr.type == TK_DOT || curr.type == TK_LBRACKET) {
                if (curr.type == TK_DOT) {
                    ...
                    if (type_id == -1) { printf("Error: Accessing member '%s' of untyped var.\n", f); exit(1); }
                    if (is_array) { printf("Error: Accessing member '%s' on array.\n", f); exit(1); }
                    int offset = find_field(type_id, f);
                    if (offset == -1) { printf("Error: Struct '%s' has no field '%s'\n", struct_defs[type_id].name, f); exit(1); }
...
void term() { factor(); while (curr.type>=TK_PLUS && curr.type<=TK_NEQ) { MyloTokenType op=curr.type; next_token(); factor(); switch(op) { case TK_PLUS: emit(OP_ADD); break; case TK_MINUS: emit(OP_SUB); break; case TK_LT: emit(OP_LT); break; case TK_GT: emit(OP_GT); break; case TK_LE: emit(OP_LE); break; case TK_GE: emit(OP_GE); break; case TK_EQ: emit(OP_EQ); break; case TK_NEQ: emit(OP_NEQ); break; default: break; } } }
void expression() { term(); if (curr.type == TK_QUESTION) { match(TK_QUESTION); emit(OP_JZ); int p1 = vm.code_size; emit(0); expression(); emit(OP_JMP); int p2 = vm.code_size; emit(0); vm.bytecode[p1] = vm.code_size; match(TK_ELSE); expression(); vm.bytecode[p2] = vm.code_size; } }
...
void for_statement() {
    ...
    if (curr.type == TK_VAR) {
        match(TK_VAR); strcpy(name, curr.text); match(TK_ID);
        if (curr.type == TK_COLON) { match(TK_COLON); char tn[MAX_IDENTIFIER]; parse_namespaced_id(tn); explicit_type = find_struct(tn); }
        is_iter = true;
    }
    else if (curr.type == TK_ID) {
        char* safe_src = src; Token safe_curr = curr;
        char temp_name[MAX_IDENTIFIER]; strcpy(temp_name, curr.text); match(TK_ID);
        if (curr.type == TK_COLON) { match(TK_COLON); char tn[MAX_IDENTIFIER]; parse_namespaced_id(tn); explicit_type = find_struct(tn); }
        if (curr.type == TK_IN) { is_iter = true; strcpy(name, temp_name); }
        else { src = safe_src; curr = safe_curr; }
    }
    ...
    if (is_iter) {
        ...
        if (is_local) {
            ...
            else { var_addr = alloc_var(true, name, explicit_type, false); created = true; }
        } else {
            char m[MAX_IDENTIFIER*2]; get_mangled_name(m, name); int glob = find_global(m);
...
void statement() {
    if (curr.type == TK_PRINT) { match(TK_PRINT); match(TK_LPAREN); if (curr.type == TK_STR) { int id = make_string(curr.text); emit(OP_PSH_STR); emit(id); match(TK_STR); } else expression(); match(TK_RPAREN); emit(OP_PRN); }
        ...
        if (curr.type == TK_COLON) {
            match(TK_COLON); char tn[MAX_IDENTIFIER]; parse_namespaced_id(tn);
            if (curr.type == TK_LBRACKET) { match(TK_LBRACKET); match(TK_RBRACKET); is_arr = true; }
            st = find_struct(tn); if (st == -1) { char m[MAX_IDENTIFIER*2]; get_mangled_name(m, tn); st = find_struct(m); }
        }
        match(TK_EQ_ASSIGN);
    ...
    else if (curr.type == TK_FOR) { for_statement(); }
    else if (curr.type == TK_ID) {
        char name[MAX_IDENTIFIER]; parse_namespaced_id(name);
        ...
        if (curr.type == TK_LPAREN) {
             ...
             if (std_idx != -1) {
                 if (std_library[std_idx].arg_count != arg_count) { printf("Error: StdLib function '%s' expects %d args\n", name, std_library[std_idx].arg_count); exit(1); }
                 emit(OP_NATIVE); emit(std_idx); emit(OP_POP); return;
             }
             char m[MAX_IDENTIFIER*2]; get_mangled_name(m, name); faddr = find_func(m);
             if (faddr != -1) { emit(OP_CALL); emit(faddr); emit(arg_count); emit(OP_POP); return; }
             printf("Error: Undefined function '%s'\n", name); exit(1);
            ...
            while (curr.type == TK_DOT || curr.type == TK_LBRACKET) {
                if (curr.type == TK_DOT) {
                    ...
                    if (curr.type == TK_EQ_ASSIGN) {
                        match(TK_EQ_ASSIGN); expression();
                        emit(OP_HSET); emit(offset); emit(type_id); emit(OP_POP); break;
                    } else {
                        emit(OP_HGET); emit(offset); emit(type_id); type_id = -1;
                    }
                } else if (curr.type == TK_LBRACKET) {
    ...
    else if (curr.type == TK_IF) { match(TK_IF); expression(); emit(OP_JZ); int p1 = vm.code_size; emit(0); match(TK_LBRACE); while(curr.type != TK_RBRACE) statement(); match(TK_RBRACE); if (curr.type == TK_ELSE) { emit(OP_JMP); int p2 = vm.code_size; emit(0); vm.bytecode[p1] = vm.code_size; match(TK_ELSE); match(TK_LBRACE); while(curr.type != TK_RBRACE) statement(); match(TK_RBRACE); vm.bytecode[p2] = vm.code_size; } else { vm.bytecode[p1] = vm.code_size; } }
    else if (curr.type == TK_RET) { match(TK_RET); expression(); emit(OP_RET); }
    else if (curr.type != TK_EOF) next_token();
}

void function() {
    match(TK_FN); char name[MAX_IDENTIFIER]; strcpy(name, curr.text); match(TK_ID); emit(OP_JMP); int p = vm.code_size; emit(0);
    ...
    match(TK_LPAREN);
    while (curr.type != TK_RPAREN) {
        char arg_name[MAX_IDENTIFIER]; strcpy(arg_name, curr.text); match(TK_ID);
        int arg_type = -1; bool arg_arr = false;
        if (curr.type == TK_COLON) {
            match(TK_COLON); char tn[MAX_IDENTIFIER]; parse_namespaced_id(tn);
            if (curr.type == TK_LBRACKET) { match(TK_LBRACKET); match(TK_RBRACKET); arg_arr = true; }
    ...
    emit(OP_PSH_NUM); int z = make_const(0.0); emit(z); emit(OP_RET); vm.bytecode[p] = vm.code_size; inside_function = ps; local_count = pl;
}

void parse_internal(char* source, bool is_import) { char* os = src; Token oc = curr; src = source; next_token(); while (curr.type != TK_EOF) { if (curr.type == TK_FN) function(); else statement(); } if (!is_import) emit(OP_HLT); src = os; curr = oc; }
...
void compile_to_c_source(const char* output_filename) {
    ...
    if (vm.str_count > 0) {
        fprintf(fp, "    for(int i=0; i<%d; i++) strcpy(vm.string_pool[i], static_strings[i]);\n    vm.str_count = %d;\n", vm.str_count, vm.str_count);
    } else {
